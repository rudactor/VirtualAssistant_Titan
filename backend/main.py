...
def best_distance(pairs: List[Tuple[object, float]]) -> Optional[float]:
    if not pairs:
        return None
    numeric_scores = [score for _, score in pairs if isinstance(score, (int, float))]
...
def filter_results(pairs: List[Tuple[object, float]], top_n: int = TOP_N) -> List[Tuple[object, float]]:
    good = []
...
def print_neighbors(title: str, results: List[Tuple[object, float]]) -> None:
    ...
    for i, (doc, score) in enumerate(results, 1):
        ...
        print(f"[#{i}] dist={score:.3f} | source={src} | page={page}")
...
def build_clarification_request(user_query: str) -> str:
    return (
        "Похоже, в базе нет точных совпадений по вашему запросу.\n"
        "Пожалуйста, уточните:\n"
        "• Что именно требуется (справка, регламент, порядок действий)?\n"
        "• Объект/подсистема (путь, тяга, вагон и т.п.).\n"
        "• Номер/раздел инструкции, если знаете.\n"
    )
